...
export async function POST(request: NextRequest) {
  try {
    ...
    const { user_name, recaptchaValue } = body
    ...
    const endpoint = `${serverUrl}/api/forgot-password`

    console.log(`🔑 Proxying forgot password to: ${endpoint}`)
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: responseData.error || 'Forgot password request failed' },
